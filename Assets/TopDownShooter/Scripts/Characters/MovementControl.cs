...
namespace TopDownShooter
{
    public class MovementControl
    {
        ...
        public MovementControl(CharacterController controller, Character characterScript,
            CharacterMovementStats movementStats, InputControl inputControl, TargetDetector targetDetector)
        {
        ...
        private void OnUpdate()
        {
            if (inputControl.Direction.magnitude >= 0.05f)
            {
                ...
                {
                    //var rotation = Quaternion.LookRotation(targetDetector.Target.transform.position);
                    //characterScript.transform.rotation = Quaternion.Slerp(characterScript.transform.rotation, rotation, 
                    //    Time.deltaTime * movementStats.turnSmoothTime);
